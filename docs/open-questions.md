...
| Q6 | Is the estate mixed, or uniformly KYC? **Working position, 2026-08-12 (danish): today uniformly KYC — only KYC services run, only they use NATS. Planned to onboard other services (KYC and non-KYC) once the KYC path is proven.** Not the D0.3 answer itself — inventory (worksheet at [`phase0/estate-inventory.md`](./phase0/estate-inventory.md)) still not started | D0.3 | [ADR-0011](./adr/0011-estate-vocabulary-versus-domain-vocabulary.md) — `tenant.id` promotion justified against the planned mixed estate; revisit if plan changes before D0.3 runs |
| Q7 | Will any 4.8 service be recompiled, or is the agent path universal? **Working position, 2026-08-12 (danish): 4.8 explicitly deferred to last — prove .NET 10 fully working first, decide 4.8 recompilation after.** Same worksheet, still not started | D0.3 | Consistent with [ADR-0012](./adr/0012-net10-first-sequencing.md)'s existing sequencing, not a change to it |
...
| Q7c | Who owns each service once discovery names them? Discovery finds processes; it does not find people. **Working position, 2026-08-11: danish owns everything by default until the inventory names services individually** — not a closed answer, since discovery (Q6/Q7) hasn't run yet and may surface services this doesn't cover | D0.3 | Reconciliation discrepancies are interviewed to a named owner. Without owners, an unaccounted-for service has nobody to ask |
