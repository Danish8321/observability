...
| Q5 | What are the performance baselines with telemetry off? Plan at [`phase0/performance-baseline.md`](./phase0/performance-baseline.md). Run 0 unattainable; Runs 1/2 not started | D0.2 | Method fixed by [ADR-0014](./adr/0014-performance-baseline-method.md). Run 0 was meant to expire first — no longer applies, see Q5d |
...
| ~~Q5c~~ | Which week is the observation window, and is it representative? | D0.2 | **Closed 2026-08-30 as moot — see Q5d, Run 0 does not happen** |
| ~~Q5d~~ | **Closed 2026-08-30 per [ADR-0027](./adr/0027-d02-baseline-has-no-data-source.md): all four Run 0 sources confirmed unreachable** (IIS/Kestrel log export, Windows perf counters, CouchDB `_stats` without admin, NATS monitoring). Run 0 is unattainable, not blocked pending access. Runs 1/2 proceed without a Run 0 cross-check | D0.2 | closed |
| ~~Q6~~ | **Closed 2026-08-24 per [ADR-0024](./adr/0024-estate-inventory-by-working-position-not-sweep.md): the working position is the answer.** None of D0.3's five sources are reachable at all — not a staffing delay, unobtainable. Estate is uniformly KYC today; other services onboarded once the KYC path is proven | — | closed |
| ~~Q7~~ | **Closed 2026-08-24, same ADR.** 4.8 recompilation deferred to last — prove .NET 10 fully working first, decide after. Consistent with [ADR-0012](./adr/0012-net10-first-sequencing.md)'s existing sequencing | — | closed |
| ~~Q7b~~ | **Answered 2026-08-10: nothing exists.** The inventory is archaeology, five-source reconciliation, and its output becomes the maintained service register per [ADR-0021](./adr/0021-service-register-is-the-coverage-denominator.md). **Superseded 2026-08-24 — the reconciliation itself is unattainable, see [ADR-0024](./adr/0024-estate-inventory-by-working-position-not-sweep.md)** | — | closed |
| ~~Q7c~~ | **Closed 2026-08-24, same ADR.** danish owns everything by default — this is now the standing answer, not a placeholder pending discovery that will not run | — | closed |
