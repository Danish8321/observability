...
**Authority**: the observability implementation plan Rev 3 owns policy/sequencing. This repo owns execution and records decisions in `docs/adr/`. Where Rev 3 and an ADR disagree, Rev 3 wins unless the ADR states the deviation explicitly (seven such deviations, listed in `docs/adr/README.md#deviations-from-rev-3`). Nothing deviates silently.
...
- Both packages target `net10.0` only. They multi-targeted `net48;net10.0` from the first commit (ADR-0012) until **ADR-0029 deferred 4.8 entirely on 2026-09-08** — out of the build, not merely unvalidated — until .NET 10 is complete and verified end to end. Do not add 4.8 code, reinstate the target framework, or work issues 11/12 without reversing ADR-0029 first.
